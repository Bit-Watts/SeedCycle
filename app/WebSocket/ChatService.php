...
class ChatService {
    ...
    public function getOrCreateConversation(int $orderId, int $buyerId, int $sellerId): int {
        // Pick the most recent conversation for this order (handles duplicates gracefully)
        $stmt = mysqli_prepare($this->conn,
            "SELECT id FROM chat_conversations
             WHERE order_id = ? AND conversation_type = 'order'
    ...
    public function getUserConversations(int $userId): array {
        $stmt = mysqli_prepare($this->conn,
            'SELECT cc.*,
             ...
             JOIN users seller ON seller.id = cc.seller_id
             WHERE (cc.buyer_id = ? OR cc.seller_id = ?)
             GROUP BY COALESCE(cc.order_id, cc.id), cc.conversation_type,
                      LEAST(cc.buyer_id, cc.seller_id), GREATEST(cc.buyer_id, cc.seller_id)
        ...
        $seen = [];
        while ($row = mysqli_fetch_assoc($result)) {
            // Deduplicate: same order+pair or same direct pair
            $key = ($row['conversation_type'] === 'direct')
                ? 'direct_' . min($row['buyer_id'], $row['seller_id']) . '_' . max($row['buyer_id'], $row['seller_id'])
                : 'order_' . $row['order_id'];
