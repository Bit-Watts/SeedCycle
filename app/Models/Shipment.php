...
class Shipment {
    ...
    public function update(int $id, string $courier, string $trackingNumber,
        ...
        $stmt = mysqli_prepare($this->conn,
            'UPDATE shipments SET courier=?, tracking_number=?, estimated_delivery=?, status=?, notes=? WHERE id=?'
        );
        mysqli_stmt_bind_param($stmt, 'sssssi', $courier, $trackingNumber, $ed, $status, $notes, $id);
        $ok = mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
